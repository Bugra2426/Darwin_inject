...
Injector::Injector() : module(0), dylib(0)
{
    module = dlopen("Your PATH",
        RTLD_NOW | RTLD_LOCAL);
    ...
    dylib = dlsym(module, "dylib");
    printf("dylib: 0x%X\n", dylib);

    if (!dylib)
    {
        fprintf(stderr, "could not locate dylib fn\n");
        return;
    }
}
...
void Injector::inject(pid_t pid, const char* lib)
{
    if (!module || !dylib)
    {
        fprintf(stderr, "failed to inject: module:0x%X dylib:0x%X\n", module, dylib);
        return;
    }
    mach_error_t err = mach_inject((mach_inject_entry)dylib, lib, strlen(lib) + 1, pid, 0);
}
